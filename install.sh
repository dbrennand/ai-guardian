...
detect_installed_agents() {
    ...
    [ -f "$HOME/.codex/config.toml" ] && grep -q "ai-guardian" "$HOME/.codex/config.toml" 2>/dev/null && agents+=("codex")
    [ ! -f "$HOME/.codex/config.toml" ] && [ -f "$HOME/.codex/hooks.json" ] && grep -q "ai-guardian" "$HOME/.codex/hooks.json" 2>/dev/null && agents+=("codex")
